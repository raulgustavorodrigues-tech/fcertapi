...
def handle_command(cmd: Dict[str, Any]) -> None:
    ...
    try:
        ...
        elif ctype == "list_tables":  res = cmd_list_tables(cid, payload)
...
def do_sync_entregas() -> None:
    ...
    try:
        sql = (
            ...
            "T1.ENDER, T1.ENDCP, T1.BAIRR, T1.MUNIC, T1.UNFED, T1.CDFILENTGDES, T1.QTFORM, "
            "T1.FLAGENTG, T1.OBSENTG, T1.VRRCB, T1.VRTOT, T1.VRTXA, T3.CDPRO "
            "FROM FC12400 T1 "
            "LEFT JOIN FC07000 T2 ON T1.CDCLIDES = T2.CDCLI "
            "LEFT JOIN (SELECT CDFILENTG, NRENTG, MIN(CDPRO) AS CDPRO "
            "FROM FC12410 GROUP BY CDFILENTG, NRENTG) T3 "
            "ON T3.CDFILENTG = T1.CDFILENTG AND T3.NRENTG = T1.NRENTG "
