...
function BancosPage() {
  ...
  const { data: databases = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase.from("databases").select("*, companies(name), agents(status, last_heartbeat_at, tunnel_url, pending_commands, agent_version)").order("created_at", { ascending: false });
